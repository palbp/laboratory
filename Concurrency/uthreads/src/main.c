...
void prepare_thread(uthread_t* pthread, void (*thread_code)()) {
    uthread_context_t* pctx = (uthread_context_t*)((pthread->stack + STACK_SIZE) - sizeof(uthread_context_t));
    pctx->rbp = 0;
...
void thread3_code() {
    puts("T3: only step");
    context_switch(&thread3, &thread2);
}

int main() {
    prepare_thread(&thread1, thread1_code);
    prepare_thread(&thread2, thread2_code);
    prepare_thread(&thread3, thread3_code);
